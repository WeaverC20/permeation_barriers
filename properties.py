...
def Cp_W(T):  # units in J/(kg*K)
    return 21.868372 + 8.068661e-03*T - 1e-06*T**2 + 1.075862e-09*T**3 + \
        1.406637e04/T**2
...
def rho_W(T):  # units in kg/m**3
    ...
    """
    T_W_0 = 293.15
    return 19250 - 2.66207e-01*(T - T_W_0) - 3.0595e-06*(T - T_W_0)**2 - \
        9.5185e-09*(T - T_W_0)**3


def thermal_cond_W(T):  # units in W/(m*K)
    return 149.441 - 45.466e-03*T + 13.193e-06*T**2 - 1.484e-09*T**3 + \
        3.866e06/(T+1)**2
...
def visc_lipb(T):  # units (Pa s)
    return 0.01555147189 - 4.827051855e-05*T + 5.641475215e-08*T**2 - \
        2.2887e-11*T**3
...
if __name__ == "__main__":
    ...
    plt.plot(
        T, thermal_cond_eurofer(T),
        label="Eurofer", color=grey_eurofer)
    plt.plot(
        T, thermal_cond_lipb(T),
        label="LiPb", color=green_lipb)
    plt.xlabel(r"T (K)")
