...
list_of_equations = [
    u1.subs(r, r_0 + L)/S2 - u2.subs(r, r_0 + L)/S1,
    D2*(r*diff(u1, r)).subs(r, r_0 + L) - D1*(r*diff(u2, r)).subs(r, r_0 + L),
    u1.subs(r, r_0) - u_0,
...
def compute_PRF_coating_outside(e, r_0, D1, D2, S1, S2, L, pressure):
    flux_with_barrier = compute_flux(e, r_0, D1, D2, S1, S2, L, pressure**0.5*S2)
    flux_wo_barrier = compute_flux(0, r_0 , D2, D2, S2, S2, L, pressure**0.5*S2)
    return flux_wo_barrier/flux_with_barrier


PRF_outside = simplify(compute_PRF_coating_outside(
